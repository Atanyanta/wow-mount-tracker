...
export default function Home() {
  ...
  const filteredMounts = useMemo(() => {
    if (!ownedIds || filter === "all") return relevantMounts;
    if (filter === "collected") return relevantMounts.filter((m) => ownedIds.has(m.id));
    return relevantMounts.filter((m) => !ownedIds.has(m.id));
  }, [filter, ownedIds, relevantMounts]);
  ...
  const { total, collected, unobtainableOwned } = useMemo(() => {
  ...
  return (
    <main>
      ...
      <CollectionSummary total={total} collected={collected} unobtainableOwned={unobtainableOwned} />
      <SearchBar onScanResult={handleScanResult} />
      <FilterBar filter={filter} onFilterChange={setFilter} disabled={!ownedIds} />
